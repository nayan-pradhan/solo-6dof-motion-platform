...
class DataProcessClass():
    ...
    def print_results(self):
        print("Platform linear acceleration:", self.platform_lin_acc_calc)
    ...
    def xyz_integration(self, xyz_d, xyz_1, dt):
        temp = []
        for i in range(3):
            temp.append(float((xyz_d[i]*dt) + xyz_1[i]))
        return temp
    ...
    def filter_error(self, raw, filtered):
        result = 0
        for i in range(len(raw)):
            for j in range(3):
                result += raw[i][j] - filtered[i][j]               

        return result


    def find_cutoff_frequency(self, data, fc_start = 5, fc_max = 500):
        fc_stop = fc_start + 10
        self.fc = fc_start
        best = None
        score = math.inf
        ...
        while self.fc < fc_stop and self.fc < fc_max: 
            new_score = 0
            for d, r in zip(data, raw_data):
                d = self.filter_data(r)
                new_score += self.filter_error(r, d)
            ...
            self.fc += 1
            # print('fc: ', self.fc, 'score: ', new_score, 'best: fc: ', best, 'score: ', score)
    ...
    def imu_data_transformation(self):
        ...
        # calculates angular acceleration from angular velocity. No need to do transformation because change of angle is same at imu and phantom
        self.imu_angular_acceleration_calculation() 
        # transforms linear acceleration from imu position to phantom position
    ...
    def imu_linear_acceleration_transformation(self):
        '''
            -61.6 mm from phantom to imu
        '''
        vec_imu = [0,0,-0.0616] # m 
        vec_phantom = [0,0,0] # m
        ...
        diff_vec = np.array(vec_imu) - np.array(vec_phantom) # m
        ...
        for i in range(len(self.imu_data)):
            ...
            transformed_linear_acc.append(np.array(raw_linear_acc) + np.array(second_derivative[i])) # substracting the additional accelereration vector from phantom to imu
            # transformed_linear_acc.append(-np.array(second_derivative[i])) # substracting the additional accelereration vector from phantom to imu
            # print(np.array(raw_linear_acc) - np.array(second_derivative[i]), second_derivative[i])
        
        self.transformted_lin_acc_imu_data = np.array(self.filter_data(transformed_linear_acc))

        # for i in range(len(self.imu_data)):
        #     for j in range(3):
        #         self.imu_data[i, j] = t_imu[i]
